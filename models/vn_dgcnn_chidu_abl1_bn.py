"""
Ablation 1: 与原始 vn_dgcnn_chidu_reg 相比，在 conv4 层加回 VN BN。
其余层（conv1/2/3）保持原始 VNLinearWithScale_PerPoint（无 BN）。
"""
import torch
...
class get_model(nn.Module):
    def __init__(self, args, normal_channel=False):
        ...
        # 消融：conv4 加回 VN BN
        self.conv4 = VNLinearWithScale_PerPoint_BN(64 * 2, 128)
        self.pool4 = VNMaxPool(128)

        self.conv5 = VNLinearLeakyReLU(16 + 32 + 64 + 128, 1024 // 3, dim=4, share_nonlinearity=True)
