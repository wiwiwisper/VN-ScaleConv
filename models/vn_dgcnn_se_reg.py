...
class get_model(nn.Module):
    def __init__(self, args, normal_channel=False):
        ...
        # VN-DGCNN Encoder
        self.conv1 = VNLinearLeakyReLU(2, 64 // 3, use_se=True)
        self.conv2 = VNLinearLeakyReLU(64 // 3 * 2, 64 // 3, use_se=True)
        self.conv3 = VNLinearLeakyReLU(64 // 3 * 2, 128 // 3, use_se=True)
        self.conv4 = VNLinearLeakyReLU(128 // 3 * 2, 256 // 3, use_se=True)

        self.conv5 = VNLinearLeakyReLU(256 // 3 + 128 // 3 + 64 // 3 * 2,
                                       1024 // 3, dim=4, share_nonlinearity=True, use_se=True)

        # self.std_feature = VNStdFeature(1024 // 3 * 2, dim=4, normalize_frame=False)
        # 增强std_feature的旋转感知能力
        self.std_feature = StdFeatureWithSE(
            in_channels=1024 // 3 * 2,
            dim=4,
            reduction_ratio=8
        )
