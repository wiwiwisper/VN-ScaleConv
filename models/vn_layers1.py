...
class StdFeatureWithSE(nn.Module):
    def __init__(self, in_channels, dim=4, reduction_ratio=8):
        super().__init__()
        self.std_feature = VNStdFeature(in_channels, dim=dim)
        self.se = VNSE(in_channels, reduction_ratio=reduction_ratio, dim=dim)

    def forward(self, x):
        # 处理标准特征
        x_std, trans = self.std_feature(x)

        # 应用SE注意力的特殊处理
        if x_std.dim() == 4:  # [B, C, 3, N]
            # 添加临时维度以兼容SE
            x_se = self.se(x_std.unsqueeze(-1)).squeeze(-1)
        else:
            x_se = self.se(x_std)

        return x_se, trans

class VNSE(nn.Module):
    def __init__(self, channel_in, reduction_ratio=16, dim=5):
        super(VNSE, self).__init__()
        self.dim = dim
        self.channel_in = channel_in
        self.reduction_ratio = reduction_ratio

        self.squeeze = nn.Sequential(
            VNLinear(channel_in, channel_in // reduction_ratio),
            VNLeakyReLU(channel_in // reduction_ratio, share_nonlinearity=True)
        )

        self.excitation = nn.Sequential(
            VNLinear(channel_in // reduction_ratio, channel_in),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        if x.dim() == 4:
            b, c, _, n = x.size()
            se = x.mean(dim=-1)  # [B, C, 3]
            se = self.squeeze(se.unsqueeze(-1).unsqueeze(-1))  # [B, C/r, 3, 1, 1]
            se = self.excitation(se).squeeze(-1)  # [B, C, 3, 1]
            return x * se.expand_as(x)

        # 原始5维处理（B,C,3,N,K）
        b, c, _, n, k = x.size()
        se = x.mean(dim=[-1, -2])
        se = self.squeeze(se.unsqueeze(-1).unsqueeze(-1))
...
class VNLinearLeakyReLU(nn.Module):
    def __init__(self, in_channels, out_channels, dim=5, share_nonlinearity=False, negative_slope=0.2,
        ...
        # 添加SE模块
        if self.use_se:  # 使用成员变量
            self.se = VNSE(out_channels, reduction_ratio, dim)
    ...
    def forward(self, x):
        ...
        if self.use_se:
            # 添加维度检查
            if p.dim() == 4 and hasattr(self.se, 'forward'):
                p = self.se(p.unsqueeze(-1)).squeeze(-1)  # 临时增加K维度
            else:
                p = self.se(p)
